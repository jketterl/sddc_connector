...
Ddc::Ddc(uint32_t new_buffersize, float freq_offset, uint16_t new_decimation) {
...
uint32_t Ddc::run(int16_t* input_samples, float* output, uint32_t length) {
    ...
    cudaMemcpy(output, output, sizeof(float) * (out_samples * 2), cudaMemcpyDeviceToHost);
